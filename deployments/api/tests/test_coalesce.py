...
Covers the per-(record, field) tiering: overridden records outrank non-overridden
ones for a field, so a source added after a reorder sorts last; per-field
isolation; same-source records ranked individually; and backward-compatible
default behaviour when no overrides are supplied.
"""

from stitch.api.coalesce import SRC_PRIORITY, coalesce_og_field_resource
from stitch.ogsi.model import GemSource, LLMSource, RMISource, WoodMacSource
...
def test_defaults_pick_highest_priority_source_per_field():
    # No overrides -> rmi(1) > gem(2) > wm(3): rmi wins where present.
    sources = [
    ...
    assert view.basin == "Alpha"  # rmi has no basin -> gem(2) wins over wm(3)
...
def test_none_and_empty_overrides_match_default_order():
    ...
    baseline = coalesce_og_field_resource(sources, SRC_PRIORITY)
