...
def _test_source_record(row: dict) -> dict:
    return {
        "kind": "provider",
        "record_id": None,
        "run_id": "test-run-id",
    ...
    async with session_factory() as session:
        ...
        for row in source_rows:
            source = OilGasFieldSourceModel(
                **row,
                source_record=_test_source_record(row),
                source_record_hash="test-hash",
...
class TestOilGasFieldSourcesLicensedSources:
    ...
    @pytest.mark.anyio
    async def test_get_licensed_source_detail_includes_raw_source_record(
        self,
        gem_only_client: AsyncClient,
        integration_session_factory: async_sessionmaker[AsyncSession],
        test_user: User,
        ...
        data = response.json()
        assert data["source"] == "gem"
        assert data["source_record"]["producer"] == "test-producer@1"
        assert data["source_record_hash"] == "test-hash"
