...
async def check_health():
    return JSONResponse(
        {"service": "stitch_llm", "status": "ok"}, status_code=HTTP_200_OK
    )
...
async def check_health_details(request: Request):
    ...
    status = "ok"
    ready = settings.azure_openai_configured or llm_backend["placeholder_mode"]
    ...
    payload = {
        "status": status,
        "ready": bool(ready and downstream["ready"]),
        "service": "stitch-llm",
        "runtime": {
